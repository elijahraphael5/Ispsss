...
async function bootstrap() {
  ...
  const tierFor = (req: Request): RateLimitRule => {
    if (req.method !== 'GET') return { limit: envLimit('RATE_LIMIT_MUTATION_PER_MIN', 200), windowMs: 60_000 };
    return { limit: envLimit('RATE_LIMIT_READ_PER_MIN', 800), windowMs: 60_000 };
  };
  ...
  app.use(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const result = await limiter.consume(`billing:${(await userKey(req)) ?? `ip:${ip}`}`, tierFor(req));
    if (!result.allowed) {
  ...
  logger.info({ port, redisUrl }, 'billing-service listening');
}
bootstrap();