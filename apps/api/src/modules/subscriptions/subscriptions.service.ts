...
export class SubscriptionsService {
  ...
  async findAll(skip = 0, take = 50, search?: string, planFilter?: string) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.subscriber.findMany({
        where,
        include: { user: { select: { id: true, email: true, phone: true } }, subscriptions: { include: { plan: true }, take: 1 }, devices: { select: { connectionType: true, ipAddress: true, status: true }, take: 5 } },
  ...
  async create(data: { userId: string; type: string; address?: string }) {
    const tenantId = await this.tenant.resolveTenant();
    const sub = await this.prisma.subscriber.create({
      data: { tenantId, userId: data.userId, type: data.type as any, address: data.address },
