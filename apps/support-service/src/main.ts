...
async function bootstrap() {
  ...
  app.use(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const result = await limiter.consume(`support:${(await userKey(req)) ?? `ip:${ip}`}`, tierFor(req));
    if (!result.allowed) {
  ...
  const port = process.env.APP_PORT ?? 4104;
  await app.listen(port);
  logger.info({ port, redisUrl }, 'support-service listening');
}
bootstrap();