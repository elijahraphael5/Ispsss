...
export interface PaymentReceiptData {
  ...
  amountKobo: number;
  reference: string;
  action?: 'renew' | 'change_plan' | 'add_plan';
  planName?: string;
}
...
<body style="margin:0;padding:0;background-color:#F1F5F9;">
  ...
  async sendPaymentReceipt(data: PaymentReceiptData): Promise<void> {
    ...
    const statusLine =
      data.action === 'change_plan'
        ? `Your plan has been changed to <strong>${data.planName ?? 'the new plan'}</strong> and is active immediately.`
        : data.action === 'add_plan'
          ? `Your new <strong>${data.planName ?? 'plan'}</strong> is now active.`
          : `Your <strong>${data.planName ?? 'internet'}</strong> plan has been renewed and your internet connection has been restored.`;

    const body = this.h(
      `<h2 style="margin:0 0 12px 0;font-size:20px;color:#0F172A;font-weight:700;">Payment Received — Plan ${data.action === 'renew' ? 'Renewed' : 'Updated'}</h2>
      <p style="margin:0 0 16px 0;color:#475569;font-size:14px;">Hello ${data.customerName},</p>
      <p style="margin:0 0 20px 0;color:#475569;font-size:14px;line-height:1.5;">${statusLine}</p>

      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F8FAFC;border-radius:8px;padding:16px;margin-bottom:20px;border:1px solid #E2E8F0;">
        <tr><td colspan="2" style="padding-bottom:8px;font-size:12px;font-weight:700;color:#0F172A;text-transform:uppercase;letter-spacing:0.5px;">Payment Details</td></tr>
        ...
        <tr><td style="padding:4px 0;color:#64748B;font-size:13px;">Amount Paid</td><td style="padding:4px 0;font-weight:700;color:#F15925;font-size:13px;" align="right">${amount}</td></tr>
        <tr><td style="padding:4px 0;color:#64748B;font-size:13px;">Transaction Reference</td><td style="padding:4px 0;font-weight:600;color:#0F172A;font-family:monospace;font-size:12px;" align="right">${data.reference}</td></tr>
      </table>

      ${this.renderButton(`${appUrl}/billing`, 'View Billing')}

      <p style="margin:0;font-size:12px;color:#94A3B8;text-align:center;">A receipt has been added to your billing history.</p>`,
      `Payment of ${amount} received for invoice ${data.invoiceNumber}`
    );

    await this.send({ to: data.email, subject: `Payment Confirmation — ${appName}`, html: body });
  }
  ...
  async sendPaymentFailed(data: PaymentFailedData): Promise<void> {
    ...
    const body = this.h(
      `<h2 style="margin:0 0 12px 0;font-size:20px;color:#DC2626;font-weight:700;">Payment Not Completed</h2>
      <p style="margin:0 0 16px 0;color:#475569;font-size:14px;">Hello ${data.customerName},</p>
      <p style="margin:0 0 20px 0;color:#475569;font-size:14px;line-height:1.5;">We were unable to process your payment of <strong>${amount}</strong> for invoice <strong>${data.invoiceNumber}</strong> (reference <strong>${data.reference}</strong>). No charge has been made.</p>

      ${this.renderButton(`${appUrl}/billing`, 'Try Again')}

      <p style="margin:0;font-size:12px;color:#94A3B8;text-align:center;">You can retry payment anytime from your billing page.</p>`,
      `Payment failed for invoice ${data.invoiceNumber}`
    );

    await this.send({ to: data.email, subject: `Payment Failed — ${appName}`, html: body });
  }
  ...
  async sendQuotationEmail(data: QuotationMailData): Promise<void> {
    ...
    await this.send({
      to: data.email,
      subject: `Quotation ${data.quotationNumber} — ${appName}`,
      html: body,
